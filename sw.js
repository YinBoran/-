...
const APP_SHELL = ["/", "/index.html", "/placeholder.jpg"];
...
self.addEventListener("fetch", function(e) {
  ...
  if (url.includes("/media/") || url.includes("/cdn-cgi/image/")) {
    e.respondWith(cacheFirst(e.request, CACHE_MEDIA));
    return;
  }
  if (url.includes("/api/")) {
...
self.addEventListener("message", async function(e) {
    ...
    for (var i = 0; i < (d.urls || []).length; i++) {
      try {
        if (!(await c2.match(d.urls[i]))) {
          var r2 = await fetch(d.urls[i]);
          if (r2.ok) await c2.put(d.urls[i], r2);
        }
      } catch (_) {}
    }
  }
  ...
  if (d.type === "RECONNECT_EVICT") {
    ...
    var vm     = d.viewMap || {};
    var sorted = keys
      .map(function(r) { return { r: r, v: vm[r.url] || 0 }; })
