// YB Platform - Cloudflare Worker v2
// Phase 2: normalizePhone, validate, auth, platform routing, feature flags

//  Hardcoded non-secret defaults 
var DEFAULTS = {
  SUPABASE_URL:       "https://jkehyntsdbkvagwxmdhj.supabase.co",
  TENANT_ID:          "6af243f2-5651-434c-9218-4c423cf9b1ce",
  SHOP_ID:            "f088d9dc-e27b-42cd-bd87-895e1fb85229",
  FB_PAGE_ID:         "4c3b46e9-ba46-4739-b7cf-a92f9bcd9efa",
  TG_PAGE_ID:         "7c7c85d1-880a-4727-a7bd-a10cc9e28977",
  FB_VERIFY_TOKEN:    "yb9999secret",
  KHQR_MERCHANT_NAME: "YB 9999",
};
...
function getEnv(env) {
  return {
    ...
    TENANT_ID:          env.TENANT_ID          || DEFAULTS.TENANT_ID,
    SHOP_ID:            env.SHOP_ID            || DEFAULTS.SHOP_ID,
    FB_PAGE_ID:         env.FB_PAGE_ID         || DEFAULTS.FB_PAGE_ID,
    TG_PAGE_ID:         env.TG_PAGE_ID         || DEFAULTS.TG_PAGE_ID,
    FB_VERIFY_TOKEN:    env.FB_VERIFY_TOKEN     || DEFAULTS.FB_VERIFY_TOKEN,
    FB_APP_SECRET:      env.FB_APP_SECRET      || "",
    FB_ACCESS_TOKEN:    env.FB_ACCESS_TOKEN    || "",
    TG_TOKEN:           env.TG_TOKEN           || "",
    TG_SECRET:          env.TG_SECRET          || "",
    KHQR_MERCHANT_NAME: env.KHQR_MERCHANT_NAME || DEFAULTS.KHQR_MERCHANT_NAME,
    KHQR_MERCHANT_ID:   env.KHQR_MERCHANT_ID   || "",
    KHQR_BANK_ACCOUNT:  env.KHQR_BANK_ACCOUNT  || "",
...
function buildCors(origin) {
  var allowed = ALLOWED_ORIGINS.includes(origin)
    ? origin : "https://yinboran.github.io";
  return {
...
var FIELD_SELECT = {
  ...
  feed:     "id,code,item_name,image_url,retail_price",
...
var CATALOG_URL = "https://yinboran.github.io/s/";
...
export default {
  ...
  async fetch(request, env_raw) {
    ...
    // Health
    if (path === "/ping") {
      return jsonResp({ ok: true, worker: "yb-v2.9", ts: Date.now() }, 200, cors);
    }
    ...
    if (path === "/debug/env") {
      return jsonResp({
        ...
        TENANT_ID:       env.TENANT_ID       ? "SET" : "MISSING",
        TG_TOKEN:        env.TG_TOKEN        ? "SET" : "MISSING",
        FB_VERIFY_TOKEN: env.FB_VERIFY_TOKEN ? "SET" : "MISSING",
      }, 200, cors);
    }
